...
interface ScriptValue { value: number }
interface ScriptRegister { register: number }
...
type WriteStringOpt = {} | { lengthPrefix: boolean } | { nullTerminated: boolean }

function writeString({ view, uint8 }: Buf, pos: number, val: string, opt: WriteStringOpt = {}) {
  ...
  if (nullTerminated && lengthPrefix) throw new Error('lengthPrefix and nullTerminated cannot both be true')
...
function createSpecialCodesTable({ offset,
  replayOid,
  stopOid,
}: { offset: number, replayOid: number, stopOid: number }) {
  /**
...
function createScriptTable({ offset, scripts: cfgScripts, tracks }: { scripts?: Record<number, ScriptLine[]>, offset: number, tracks?: Track[] }) {
  ...
  return {
    ...
    write({ uint8, view }: Buf) {
      ...
      for (const item of items) if (item.encoded != null) uint8.set(item.encoded, item.scriptOffset)
    }
  }
}
...
function createHeader({ comment: cfgComment, language: lang, productId }: Omit<GmeBuildConfig, 'tracks'>) {
  ...
  const items = {
    ...
    specialCodesOffset: id<LayoutItem & { val: number }>({
      val: 0,
      write({ view }) { view.setUint32(0x0094, this.val, true) }
    })
    /**
