...
export function OptionsPanel() {
  ...
  const saveProject = useCallback(async () => {
    ...
    for (const k of (await keys()) as string[]) {
      ...
      writer.write(JSON.stringify([k, data], (_key, value) => {
        if (value instanceof Uint8Array)
          return 'uint8array:base64:' + uInt8ArrayToBase64(value);
        if (value instanceof ArrayBuffer)
          return 'arraybuffer:base64:' + uInt8ArrayToBase64(new Uint8Array(value))
        return value;
      }));
      writer.write('\n')
    }
            ...
            const [key, data] = JSON.parse(line, (_k: string, value: unknown) => {
              if (typeof value === 'string') {
                ...
                if (value.startsWith('arraybuffer:base64:'))
                  return fromBase64(value.substring('arraybuffer:base64:'.length)).buffer
              }
              return value
            });
            await set(key, data)
  ...
  const deleteProject = () => {
    confirm(i18n`Are you sure you want to delete this project?`)
      .then((confirmed) => {
        if (!confirmed) return
        return clear().then(() => {
  ...
  return (
    <>
      ...
      <FormField label={''} >
        <Group gap={4}>
          <Button leftSection={<Download {...iconStyle} />} onClick={saveProject} >
            {i18n`Save Project`}
          </Button>
          <Button leftSection={<Upload {...iconStyle} />} onClick={openProject} >
            {i18n`Open Project`}
          </Button>
          <Button color="red" onClick={deleteProject} leftSection={<Trash {...iconStyle} />} >
            {i18n`Delete Project`}
          </Button>
        </Group>
      </FormField>

      <FormField label={i18n`Product ID`} tooltip={i18n`ID of the product. Used to generate the OID codes.`}>
        <NumberInput min={1} max={1000} mt={0.75} {...form.getInputProps('productId')} />
      </FormField>

      <FormField label={i18n`OID Code Resolution`} tooltip={i18n`Resolution at which OID codes will be generated.`}>
        <NumberInput mt={0.75} {...form.getInputProps('oidCodeResolution')} rightSection='DPI' />
      </FormField>

      <FormField label={i18n`OID Pixel Size`} tooltip={i18n`Number of pixels (squared) for each dot in the OID code.`}>
        <NumberInput mt={0.75} {...form.getInputProps('oidCodePixelSize')} rightSection='px' />
      </FormField>

      <FormField label={i18n`"Play All" Code`} tooltip={i18n`OID code that will trigger the playback of all tracks.`}>
        <NumberInput min={1} max={0xffff} mt={0.75} {...form.getInputProps('playAllOid')} />
      </FormField>

      <FormField label={i18n`Replay Code`} tooltip={i18n`OID code that will trigger the replay of the last track.`}>
        <NumberInput min={1} max={0xffff} mt={0.75} {...form.getInputProps('replayOid')} />
      </FormField>

      <FormField label={i18n`Stop Code`} tooltip={i18n`OID code that will stop the current track.`}>
        <NumberInput min={1} max={0xffff} mt={0.75} {...form.getInputProps('stopOid')} />
      </FormField>
